...
if __name__ == '__main__':
    ...
    if args.out:
        ...
        torch.save(model, f'./tmp/{args.dataset}/bprmf/{args.prepro}_{args.test_method}_bprmf.pt')
    ...
    pred_csv.to_csv(f'{result_save_path}{args.dataset}_{args.prepro}_bprmf.csv', index=False)
    
    res_csv = pd.DataFrame(data=u_result)
    res_csv.to_csv(f'{result_save_path}{args.dataset}_{args.prepro}_result_mf.csv', index=False)
    ...
    res.to_csv(f'{result_save_path}{args.prepro}_bprmf.csv', 
               index=False)
    print('='* 20, ' Done ', '='*20)

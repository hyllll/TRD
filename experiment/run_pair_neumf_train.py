...
if __name__ == '__main__':
    ...
    model = PairNeuMF(user_num, item_num, args.factors, args.num_layers, args.dropout, 
                      args.lr, args.epochs, args.lamda, args.model_name, GMF_model, MLP_model, 
                      args.gpu, args.loss_type, use_cuda=args.use_cuda)
    model.fit(train_loader)
    if args.out:
        if not os.path.exists(f'./tmp/{args.dataset}/neumf/'):
            os.makedirs(f'./tmp/{args.dataset}/neumf/')
        torch.save(model, f'./tmp/{args.dataset}/neumf/{args.prepro}_{args.factors}_neumf_train.pt')
    ...
    pred_csv.to_csv(f'{result_save_path}{args.dataset}_{args.prepro}_{args.factors}_neumf_train.csv', index=False)

    res_csv = pd.DataFrame(data=u_result)
    res_csv.to_csv(f'{result_save_path}{args.dataset}_{args.prepro}_{args.factors}_result_neumf_train.csv', index=False)
    ...
    #save binary-interaction list to csv file
    pred_csv = pd.DataFrame(data=u_binary)
    pred_csv.to_csv(f'{result_save_path}{args.dataset}_{args.prepro}_{args.factors}_neumf_train1.csv', index=False)
    
    res_csv = pd.DataFrame(data=u_result)
    res_csv.to_csv(f'{result_save_path}{args.dataset}_{args.prepro}_{args.factors}_result_neumf_train1.csv', index=False)
    ...
    res.to_csv(f'{result_save_path}{args.prepro}_{args.factors}_neumf.csv', 
               index=False)
    print('='* 20, ' Done ', '='*20)
